...
void iniciar_sessao(char *message){
  ...
  strncpy(pipe_req, &message[1], 41); // Copiar os primeiros 40 caracteres após o número
  pipe_req[40] = '\0';             // Adicionar terminador nulo
  strncpy(pipe_resp, &message[41], 41); // Copiar os próximos 40 caracteres
  pipe_resp[40] = '\0';              // Adicionar terminador nulo
  strncpy(pipe_notif, &message[80], 41); // Copiar os últimos 40 caracteres
  pipe_notif[40] = '\0';  
  printf("code: %d\n",code);
  printf("pipe req: %s\n",pipe_req);
  printf("pipe resp: %s\n",pipe_resp);
