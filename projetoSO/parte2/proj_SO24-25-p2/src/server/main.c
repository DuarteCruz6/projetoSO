...
void iniciar_sessao(char *message){
  ...
  strncpy(pipe_req, &message[1], 39); // Copiar os primeiros 40 caracteres após o número
  pipe_req[40] = '\0';             // Adicionar terminador nulo
  strncpy(pipe_resp, &message[40], 39); // Copiar os próximos 40 caracteres
  pipe_resp[40] = '\0';              // Adicionar terminador nulo
  strncpy(pipe_notif, &message[79], 39); // Copiar os últimos 40 caracteres
  pipe_notif[40] = '\0';  
  printf("code: %d\n",code);
  printf("pipe req: %s\n",pipe_req);
  printf("pipe resp: %s\n",pipe_resp);
...
void sinalDetetado() {
  ...
  while (userAtual!=NULL){
    ...
    pthread_mutex_unlock(&bufferThreads->buffer_mutex); //desbloquear o buffer 
    //fechar todos os pipes do cliente
    int pipe = open(cliente->req_pipe_path, O_RDONLY);
    close(pipe);
    pipe = open(cliente->resp_pipe_path, O_RDONLY);
    close(pipe);
    pipe = open(cliente->notif_pipe_path, O_RDONLY);
    close(pipe);
    free(cliente);
